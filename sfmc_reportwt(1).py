...
KPI_ICONS = {
    "Total Delivered": os.path.join(_ASSETS_DIR, "icon_delivered.png"),
    "Total Opens":     os.path.join(_ASSETS_DIR, "icon_opens.png"),
    "OR":              os.path.join(_ASSETS_DIR, "icon_pct.png"),
    "Total Clicks":    os.path.join(_ASSETS_DIR, "icon_clicks.png"),
    "CTR":             os.path.join(_ASSETS_DIR, "icon_pct.png"),
}

def add_kpi_icon(slide, label, x, y, size):
    """Add a small white-bg icon to the left of a KPI block, if available."""
    path = KPI_ICONS.get(label)
...
def slide2(prs, df):
    ...
    for i, ta_code in enumerate(TA_ORDER):
        ...
        for ki,(lbl,val) in enumerate(kpis):
            icon_path = KPI_ICONS.get(lbl)
            kx = x+ki*kw
            has_icon = bool(icon_path and os.path.exists(icon_path))
            if has_icon:
                # estimate label text width to center icon+label as a group
                text_w_est = 0.06 * len(lbl)
                group_w = small_icon + 0.04 + text_w_est
                gx = kx + (kw-group_w)/2
                sl.shapes.add_picture(icon_path,
                    Inches(gx), Inches(y+0.335),
                    Inches(small_icon), Inches(small_icon))
                lbl_x = gx + small_icon + 0.04
                lbl_w = text_w_est
            ...
            # Label — Roboto
            tb_lbl = sl.shapes.add_textbox(Inches(lbl_x), Inches(y+0.32), Inches(lbl_w), Inches(0.18))
            tf_lbl = tb_lbl.text_frame
            ...
            r_lbl.font.color.rgb = GREY; r_lbl.font.name = "Roboto"
            # Value — Roboto Condensed
            tb_val = sl.shapes.add_textbox(Inches(x+ki*kw), Inches(y+0.48), Inches(kw), Inches(0.24))
            tf_val = tb_val.text_frame
